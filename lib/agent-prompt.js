...
| Read a text file | \`read_file\` (optional line_offset / line_limit / grep) |
| Process tree | \`get_process_tree\` (pid or exact name) |
| Quit a macOS app | \`quit_app\` (exact name; prefer over killing GUI-supervised daemons) |
...
| Run shell command | \`run_command\` (cwd = session cwd or project; may background after wait_ms) |
| Poll / slice command output | \`get_command_output\` (job_id; only_new; stream; grep; line_offset / line_limit) |
| Stop background command | \`stop_command\` (job_id; kills process group) |
| Safer kill | \`terminate_process\` (job_id or session-owned pid) |
| Stop every session job | \`stop_all_session_processes\` |
| List shell jobs | \`list_commands\` |
| Persistent session env/cwd/venv | \`get_session\` / \`set_session\` (not a PTY) |
| Wait for port / HTTP / job / file / output | \`wait_until\` |
| HTTP to local servers | \`http_request\` (loopback unless allow_public) |
...
- \`run_command\` runs in the **session cwd** if set, otherwise the project root (relative \`cwd\` only unless sandbox is off).
- There is **no interactive PTY**. Persist environment with \`set_session\` (\`env\`, \`cwd\`, \`venv\`) instead of \`source ~/.venvs/.../bin/activate\` in every command. \`get_session\` shows the overlay.
...
- Jobs are **session-owned**: spawned in a process group and tagged with \`PASTEPATCH_SESSION\` / \`PASTEPATCH_JOB\`. \`stop_command\` kills the group and leftovers. \`terminate_process\` refuses PIDs this session did not launch. \`stop_all_session_processes\` cleans up everything from this MCP process.
...
- If the reply says output was truncated and you need more detail, call **\`get_command_output\`** with the same \`job_id\` — do **not** re-run the command just to see more logs:
  ...
  - \`grep\`, \`grep_context\`, \`line_offset\`, \`line_limit\` — slice server-side
...
- Use \`read_file\` with \`line_offset\` / \`line_limit\` / \`grep\` instead of \`sed\` / \`head\` via the shell.
- Use \`get_process_tree\` to see parents/children before killing anything. If a GUI app supervises a server (Ollama), call \`quit_app\` with the exact app name — do not only kill the child.
- Use \`wait_until\` instead of \`sleep\` + \`ps\` + \`tail\` (port_open, http_status, job_exits, process_exits, file_size_stable, output_matches).
- Use \`http_request\` for loopback APIs (Ollama, mlx-vlm). Non-loopback requires \`allow_public: true\`.
