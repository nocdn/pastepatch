...
export function createPastepatchMcpServer({
  ...
  server.registerTool(
    "view_image",
    {
      title: "View image",
      description:
        "Load an image file from the remote project and return it as MCP image content (so you can actually see it), plus text metadata (format, dimensions, size, whether resized/re-encoded). Use for screenshots, UI mocks, design assets, and test failure PNGs. Do not use read_file on binary images. Large images are auto-downscaled for vision models (default max side 2048px).",
      inputSchema: {
        ...
        max_dimension: z
          ...
          .optional()
          .describe(
            "Max longest side in pixels after resize (default 2048, min 64, max 4096). Smaller values reduce payload size.",
          ),
      },
      annotations: {
        readOnlyHint: true,
    ...
    async ({ path: filePath, max_dimension: maxDimension }) =>
      runTool("view_image", { path: filePath }, async (note) => {
        ...
        const dims =
          meta.resized &&
          (meta.original_width !== meta.width || meta.original_height !== meta.height)
            ? `${meta.original_width}×${meta.original_height} → ${meta.width}×${meta.height}`
            : `${meta.width}×${meta.height}`;
        const formats =
          meta.reencoded && meta.original_format !== meta.format
            ? `${meta.original_format} → ${meta.format}`
            : meta.format;
        const fileSize =
          meta.original_bytes !== meta.bytes
            ? `file ${formatBytes(meta.original_bytes)} → image ${formatBytes(meta.bytes)}`
            : `image ${formatBytes(meta.bytes)}`;

        note(
          `${dims}; ${formats}; ${fileSize}; sent ${formatBytes(payloadBytes)} base64` +
            `; resized ${meta.resized ? "yes" : "no"}; reencoded ${meta.reencoded ? "yes" : "no"}`,
        );
