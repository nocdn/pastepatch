...
export function createPastepatchMcpServer({
  ...
  const server = new McpServer(
    ...
    {
      instructions: [
        `You are the model in ChatGPT. pastepatch MCP tools (via a tunnel) let you read, search, and edit real files on the user's machine under ${resolvedRoot}.`,
  ...
  app.all("/mcp", async (req, res) => {
    try {
      ...
      if (sessionId && transports[sessionId]) {
        ...
        const server = createPastepatchMcpServer({ root, version, logger, verbose, allowOutside });
